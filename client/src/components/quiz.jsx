...
export default function Quiz({quizData}) {
  ...
  // const [finalAnswers, setFinalAnswers] = useState([])// submit form
  ...
  const handleMultipleAnswer = (questionId, optionIndex) => {
    setAnswers((prevAnswers) => {
      ...
      return {
        ...prevAnswers,
        [questionId]: {questionType: "MultiCorrect",
  ...
  const handleIntegerAnswer = (e, questionId) => {
    ...
    if (!isNaN(numValue)) {
      ...
      setAnswers({ ...answers, [questionId]: {questionType: "Integer",response:numValue} });
    } else {
  ...
  const handleShortAnswer = (e) => {
    ...
    setAnswers((prevAnswers) => ({
      ...prevAnswers,
      [questions[currentQuestion]._id]: { questionType: "ShortAnswer", response: value }
    }));
  };
  ...
  const handleSubmitQuiz = () => {
    ...
    // const formattedAnswers = Object.entries(answers).map(([questionId, selectedIndexes]) => ({
    //   questionId,
    //   selectedOptionIndexes: selectedIndexes,
    // }))

    // setFinalAnswers(formattedAnswers)
    setShowResults(true)
    setIsTimerRunning(false)
  ...
  return (
    <div
      className="w-full max-w-6xl mx-auto flex flex-col md:flex-row gap-4 relative">
      <Button
        variant="outline"
        size="icon"
